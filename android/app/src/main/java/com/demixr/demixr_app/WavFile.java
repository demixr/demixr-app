...
public class WavFile {
    ...
    private boolean
            wordAlignAdjust; // Specify if an extra byte at the end of the data chunk is required
                             // for word
    // alignment
        ...
        while (true) {
            ...
            if (chunkID == FMT_CHUNK_ID) {
                ...
                if (wavFile.validBits > 64)
                    throw new WavFileException(
                            "Valid Bits specified in header is greater than 64, this is greater"
                ...
                if (wavFile.bytesPerSample * wavFile.numChannels != wavFile.blockAlign)
                    throw new WavFileException(
                            "Block Align does not agree with bytes required for validBits and"
                                + " number of channels");
    ...
    public int writeFrames(float[] sampleBuffer, int numFramesToWrite)
            throws IOException, WavFileException {
        return writeFrames(sampleBuffer, 0, numFramesToWrite);
    }
        ...
        try {
            for (String filename : args) {
                ...
                WavFile writeWavFile =
                        newWavFile(
                                new File("out.wav"), numChannels, numFrames, validBits, sampleRate);
