...
namespace ffmpegkit {
    ...
    class AbstractSession : public Session, std::enable_shared_from_this<AbstractSession> {
        ...
        private:
            ...
            // Guards the mutable state below that is written by the ffmpeg worker
            // / async callback thread and read from the platform thread:
            // _logs (contents), _state, _returnCode, _startTime, _endTime,
            // _failStackTrace.
