...
static std::mutex messagesInTransitMutex;
static std::condition_variable messagesInTransitMonitor;
class CallbackData;
...
thread_local long globalSessionId = 0;
...
static std::thread* callbackThreadPtr = nullptr;
    ...
    while(redirectionEnabled) {
        try {
            ...
            if (callbackData != nullptr) {
                ...
                // Wake any thread waiting in waitForMessagesInTransmit now that an
                // in-transit message has been fully processed.
                messagesInTransitMonitor.notify_all();
...
void ffmpegkit::FFmpegKitConfig::enableRedirection() {
    ...
    if (callbackThreadPtr != nullptr) {
        // A previous consumer thread should already have stopped (disableRedirection
        // joins it). Join defensively before replacing it so we never run two.
        if (callbackThreadPtr->joinable()) {
            callbackThreadPtr->join();
        }
        delete callbackThreadPtr;
        callbackThreadPtr = nullptr;
    }
    callbackThreadPtr = new std::thread(callbackThreadFunction);
...
void ffmpegkit::FFmpegKitConfig::disableRedirection() {
    ...
    // Wait for the consumer thread to observe the cleared flag and exit, so a
    // later enableRedirection() never leaves two consumer threads running.
    if (callbackThreadPtr != nullptr && callbackThreadPtr->joinable()) {
        callbackThreadPtr->join();
    }
...
#ifdef _WIN32
    const char *parentDirectory = std::getenv("USERPROFILE");
    if (parentDirectory == NULL) {
        parentDirectory = std::getenv("TEMP");
        if (parentDirectory == NULL) {
            parentDirectory = ".";
        }
    }
...
std::shared_ptr<std::string> ffmpegkit::FFmpegKitConfig::registerNewFFmpegPipe() {
...
#ifdef _WIN32
    std::string pipeName = std::string("\\\\.\\pipe\\") + FFmpegKitNamedPipePrefix + std::to_string(pipeIndexGenerator++);
    std::shared_ptr<std::string> newFFmpegPipePath = std::make_shared<std::string>(pipeName);
    ...
    if (hPipe == INVALID_HANDLE_VALUE) {
        std::cout << "Failed to register new FFmpeg pipe " << pipeName << ". Error: " << GetLastError() << "." << std::endl;
        return nullptr;
    }
...
void ffmpegkit::FFmpegKitConfig::enableLogCallback(const ffmpegkit::LogCallback callback) { logCallback = callback; }
void ffmpegkit::FFmpegKitConfig::enableStatisticsCallback(const ffmpegkit::StatisticsCallback callback) { statisticsCallback = callback; }
void ffmpegkit::FFmpegKitConfig::enableFFmpegSessionCompleteCallback(const FFmpegSessionCompleteCallback completeCallback) { ffmpegSessionCompleteCallback = completeCallback; }
ffmpegkit::FFmpegSessionCompleteCallback ffmpegkit::FFmpegKitConfig::getFFmpegSessionCompleteCallback() { return ffmpegSessionCompleteCallback; }
void ffmpegkit::FFmpegKitConfig::enableFFprobeSessionCompleteCallback(const FFprobeSessionCompleteCallback completeCallback) { ffprobeSessionCompleteCallback = completeCallback; }
ffmpegkit::FFprobeSessionCompleteCallback ffmpegkit::FFmpegKitConfig::getFFprobeSessionCompleteCallback() { return ffprobeSessionCompleteCallback; }
void ffmpegkit::FFmpegKitConfig::enableMediaInformationSessionCompleteCallback(const MediaInformationSessionCompleteCallback completeCallback) { mediaInformationSessionCompleteCallback = completeCallback; }
ffmpegkit::MediaInformationSessionCompleteCallback ffmpegkit::FFmpegKitConfig::getMediaInformationSessionCompleteCallback() { return mediaInformationSessionCompleteCallback; }

ffmpegkit::Level ffmpegkit::FFmpegKitConfig::getLogLevel() { return static_cast<ffmpegkit::Level>(configuredLogLevel); }
void ffmpegkit::FFmpegKitConfig::setLogLevel(const ffmpegkit::Level level) { configuredLogLevel = level; }
...
int ffmpegkit::FFmpegKitConfig::getSessionHistorySize() { return sessionHistorySize; }
...
std::shared_ptr<ffmpegkit::Session> ffmpegkit::FFmpegKitConfig::getLastCompletedSession() {
    ...
    for(auto rit=sessionHistoryList.rbegin(); rit != sessionHistoryList.rend(); ++rit) {
        auto session = *rit;
        if (session->getState() == SessionStateCompleted) { return session; }
    }
    return nullptr;
}
...
std::shared_ptr<std::list<std::shared_ptr<ffmpegkit::FFmpegSession>>> ffmpegkit::FFmpegKitConfig::getFFmpegSessions() {
    ...
    for(auto it=sessionHistoryList.begin(); it != sessionHistoryList.end(); ++it) {
        auto session = *it;
        if (session->isFFmpeg()) { ffmpegSessions->push_back(std::static_pointer_cast<ffmpegkit::FFmpegSession>(session)); }
    }
    lock.unlock();
...
std::shared_ptr<std::list<std::shared_ptr<ffmpegkit::FFprobeSession>>> ffmpegkit::FFmpegKitConfig::getFFprobeSessions() {
    ...
    for(auto it=sessionHistoryList.begin(); it != sessionHistoryList.end(); ++it) {
        auto session = *it;
        if (session->isFFprobe()) { ffprobeSessions->push_back(std::static_pointer_cast<ffmpegkit::FFprobeSession>(session)); }
    }
    lock.unlock();
...
std::shared_ptr<std::list<std::shared_ptr<ffmpegkit::MediaInformationSession>>> ffmpegkit::FFmpegKitConfig::getMediaInformationSessions() {
    ...
    for(auto it=sessionHistoryList.begin(); it != sessionHistoryList.end(); ++it) {
        auto session = *it;
        if (session->isMediaInformation()) { mediaInformationSessions->push_back(std::static_pointer_cast<ffmpegkit::MediaInformationSession>(session)); }
    }
    lock.unlock();
...
std::shared_ptr<std::list<std::shared_ptr<ffmpegkit::Session>>> ffmpegkit::FFmpegKitConfig::getSessionsByState(const SessionState state) {
    ...
    for(auto it=sessionHistoryList.begin(); it != sessionHistoryList.end(); ++it) {
        auto session = *it;
        if (session->getState() == state) { sessions->push_back(session); }
    }
    lock.unlock();
    return sessions;
}

ffmpegkit::LogRedirectionStrategy ffmpegkit::FFmpegKitConfig::getLogRedirectionStrategy() { return globalLogRedirectionStrategy; }
void ffmpegkit::FFmpegKitConfig::setLogRedirectionStrategy(const LogRedirectionStrategy logRedirectionStrategy) { globalLogRedirectionStrategy = logRedirectionStrategy; }
...
void ffmpegkit::FFmpegKitConfig::waitForMessagesInTransmit(const int milliseconds) {
    std::unique_lock<std::mutex> lock(messagesInTransitMutex);
    messagesInTransitMonitor.wait_for(lock, std::chrono::milliseconds(milliseconds));
}
...
std::list<std::string> ffmpegkit::FFmpegKitConfig::parseArguments(const std::string& command) {
    ...
    for (int i = 0; i < command.size(); i++) {
        char previousChar;
        if (i > 0) { previousChar = command[i - 1]; } else { previousChar = 0; }
        char currentChar = command[i];

        if (currentChar == ' ') {
            if (singleQuoteStarted || doubleQuoteStarted) { currentArgument += currentChar; }
            else if (currentArgument.size() > 0) { argumentList.push_back(currentArgument); currentArgument = ""; }
    ...
    if (currentArgument.size() > 0) { argumentList.push_back(currentArgument); }

    return argumentList;
}

std::string ffmpegkit::FFmpegKitConfig::argumentsToString(std::shared_ptr<std::list<std::string>> arguments) {
    if (arguments == nullptr) { return "null"; }
    std::string string;
    for(auto it=arguments->begin(); it != arguments->end(); ++it) {
        auto argument = *it;
        if (it != arguments->begin()) { string += " "; }
        string += argument;
    }
    return string;
}
