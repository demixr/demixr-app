...
void ffmpegkit::AbstractSession::waitForAsynchronousMessagesInTransmit(const int timeout) const {
    ...
    // Wait on the shared callback monitor, which is signalled each time an
    // in-transit message is consumed. The 100ms cap bounds the wait so we
    // re-check the expiry and the in-transit count periodically.
    while (this->thereAreAsynchronousMessagesInTransmit() && (std::chrono::system_clock::now() < expireTime)) {
        ffmpegkit::FFmpegKitConfig::waitForMessagesInTransmit(100);
    }
}
...
std::shared_ptr<std::list<std::shared_ptr<ffmpegkit::Log>>> ffmpegkit::AbstractSession::getLogs() const {
    // Return a snapshot copy so callers can iterate safely while the worker
    // thread may still be appending log entries. Log entries are immutable
    // shared_ptrs, so a shallow copy of the list is a stable view.
    std::lock_guard<std::mutex> lock(_mutex);
...
void ffmpegkit::AbstractSession::cancel() {
    ...
    {
        std::lock_guard<std::mutex> lock(_mutex);
        running = (_state == SessionStateRunning);
    }
    // Call out without holding the lock to avoid re-entrant deadlock.
    if (running) {
