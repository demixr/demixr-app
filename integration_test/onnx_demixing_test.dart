...
void main() {
  ...
  testWidgets('htdemucs ONNX engine produces 4 parity-matching stems',
    ...
    final stems = await engine.separate(
      ...
      providerOverride: [OrtProvider.CPU], // deterministic parity vs reference
    );
    stopwatch.stop();
    // ignore: avoid_print
    ...
    for (final stem in stems.keys) {
      ...
      expect((samples.length - ref.length).abs(), lessThan(DemucsConfig.channels),
      ...
      // ignore: avoid_print
      print('$stem parity: maxDiff=$maxDiff LSB, rms=${rms.toStringAsFixed(2)} LSB');
  ...
  testWidgets('htdemucs_6s ONNX engine produces 6 parity-matching stems',
      (tester) async {
    final fixtures = await _fixturesDir();
    ...
    final outDir = p.join((await getTemporaryDirectory()).path, 'onnx_out_6s');
    await Directory(outDir).create(recursive: true);
